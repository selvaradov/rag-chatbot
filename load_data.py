...
def process_csv(file_path):
    ...
    for i, row in df.iterrows():
        ...
        for column in df.columns[1:]:  # Skip the Date column
            ...
            assert isinstance(content, str), f"Content for column {column} in row {i} is not a string: {content}"
...
def process_unstructured(path):
    if os.path.isfile(path):
        ...
        loader = DirectoryLoader(path, glob=["**/*.md", "**/*.txt"], loader_cls=TextLoader)
    else:
